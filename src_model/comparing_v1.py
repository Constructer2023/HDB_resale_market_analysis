...
import pandas as pd
from src_model.config import comparison_path_v1, model_dir
from src_model.testing_v1 import evaluate_all_saved_models, get_feature_importance
from src_model.training_v1 import train_all_models
from src_model.utils import load_model, setup_logging
...
def run_full_comparison(retrain: bool = False, model_names: Optional[List[str]] = None, save: bool = True):
    ...
    3) Save model_comparison_v1.csv
    """
    setup_logging()
    if retrain:
    ...
    if save:
        comparison_df.to_csv(comparison_path_v1, index = False)
        logger.info('Comparison table saved -> %s', comparison_path_v1)
    cols = [
        c
...
def summary_report(comparison_df: Optional[pd.DataFrame] = None):
    if comparison_df is None:
        if not os.path.exists(comparison_path_v1):
            return 'No comparison results. Run run_full_comparison() first.'
        comparison_df = pd.read_csv(comparison_path_v1)

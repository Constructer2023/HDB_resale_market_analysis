...
import numpy as np
import pandas as pd
from src_model.config import model_dir, target_col
from src_model.training_v2 import load_model_ready_data
from src_model.utils import (
    list_saved_models_v2,
    load_model,
    regression_metrics,
    setup_logging
)
...
def evaluate_on_test(model_name: str, X_test: Optional[pd.DataFrame] = None, y_test: Optional[pd.Series] = None):
    """
    Load model/{name}_v2.joblib and score on the 2026 test set.
    """
    setup_logging()
